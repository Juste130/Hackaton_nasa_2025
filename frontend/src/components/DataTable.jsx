...
const DataTable = ({ publications, selectedPublications, onPublicationSelect }) => {
  ...
  return (
    <div className="data-table">
          ...
          <thead>
            <tr>
                ...
                <input
                  ...
                  onChange={() => {
                    if (selectedPublications.length === publications.length) {
                      publications.forEach(pub => onPublicationSelect(pub.id));
                    } else {
                      publications.forEach(pub => onPublicationSelect(pub.id));
                    }
                  }}
              ...
              <th onClick={() => handleSort('title')} className="sortable">
                Title <span className="sort-icon"><SortIcon field="title" /></span>
              </th>
              <th onClick={() => handleSort('authors')} className="sortable">
                Authors <span className="sort-icon"><SortIcon field="authors" /></span>
              </th>
              ...
              <th>Organisms</th>
              <th>Phenomena</th>
              <th onClick={() => handleSort('citations')} className="sortable">
                Citations <span className="sort-icon"><SortIcon field="citations" /></span>
              </th>
            </tr>
          </thead>
          <tbody>
            {paginatedPublications.map(publication => (
              <tr 
                key={publication.id} 
                className={selectedPublications.includes(publication.id) ? 'selected' : ''}
              >
                <td>
                  <input
                    type="checkbox"
                ...
                <td className="citations-cell">
                  <span className="citations-count">{publication.citations}</span>
                </td>
              </tr>
            ))}
          </tbody>
      ...
      <div className="table-footer">
        <p>Showing {paginatedPublications.length} of {sortedPublications.length} publications</p>
      </div>
    </div>
  );
};

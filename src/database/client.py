...
Asynchronous client for CRUD operations with PostgreSQL and SQLAlchemy
"""
...
class DatabaseClient:
    """Asynchronous client for NASA Bioscience Publications database"""
    ...
    async def create_publication(self, publication_data: Dict[str, Any]) -> uuid.UUID:
        """Create a publication with all its relations"""
        async with self.async_session() as session:
            try:
                # 1. Create main publication
                pub = Publication(
                ...
                logger.info(f"Publication {pub.pmcid} created with ID {pub.id}")
                return pub.id
                
            except Exception as e:
                await session.rollback()
                logger.error(f"Error creating publication {publication_data.get('pmcid', 'unknown')}: {e}")
                raise
    
    async def update_publication(self, publication_id: uuid.UUID, update_data: Dict[str, Any]) -> bool:
        """Update a publication with provided data"""
        try:
            async with self.async_session() as session:
    ...
    async def get_publication_by_pmcid(self, pmcid: str) -> Optional[Dict[str, Any]]:
        """Get a publication by its PMCID"""
        try:
            async with self.async_session() as session:
            ...
            logger.error(f"Error retrieving publication {pmcid}: {e}")
            return None
    ...
    async def delete_publication(self, pub_id: uuid.UUID) -> bool:
        ...
        async with self.async_session() as session:
            try:
                ...
                await session.rollback()
                logger.error(f"Error deleting publication {pub_id}: {e}")
                return False
    ...
    async def _add_keywords_to_publication(self, session: AsyncSession, pub_id: uuid.UUID, keywords_data: List[str]):
        ...
        for keyword_text in keywords_data:
            if keyword_text and keyword_text.strip():
                ...
                if len(cleaned) > 1:  # At least 2 characters
                    unique_keywords.add(cleaned)
        
        logger.debug(f"Adding {len(unique_keywords)} unique keywords")
        ...
        for keyword_text in unique_keywords:
            try:
                ...
                if not existing_relation.scalar_one_or_none():
                    ...
                    logger.debug(f"Keyword '{keyword_text}' already linked to publication")
                    
            except Exception as e:
                logger.warning(f"Error adding keyword '{keyword_text}': {e}")
                continue

    async def _add_mesh_terms_to_publication(self, session: AsyncSession, pub_id: uuid.UUID, mesh_terms_data: List[str]):
        """Add MeSH terms to a publication"""
        if not mesh_terms_data:
            return
            
        # Deduplicate MeSH terms
    ...
    async def _add_text_sections_to_publication(self, session: AsyncSession, pub_id: uuid.UUID, sections_data: Dict[str, str]):
        ...
        for i, (section_name, content) in enumerate(sections_data.items()):
            ...
            # Section name validation and cleaning
            if not section_name or not section_name.strip():
                section_name = f"Section_{i+1}"
                logger.warning(f"Empty section name replaced by {section_name}")
            
            # Clean and limit section name length
    ...
    async def _publication_to_dict(self, session: AsyncSession, publication: Publication) -> Dict:
        """Convert a publication to dictionary with all relationships"""
        # Load authors
...
async def main():
    ...
    try:
        ...
        print(f"Publication created with ID: {pub_id}")
        
        # Search publication
        result = await client.get_publication_by_pmcid('PMC1234567')
        print(f"Publication found: {result['title']}")

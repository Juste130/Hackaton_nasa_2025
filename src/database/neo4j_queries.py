...
class GapAnalyzer:
    ...
    def calculate_completeness_matrix_paginated(self, 
                                                ...
                                                limit: int = 5) -> Dict:
        """
        Paginated version to process biological systems in batches
        Ideal for REST APIs with Neo4j Aura (limited memory)
        
        Args:
            skip: Number of systems to skip
            limit: Number of systems to return (recommended: 5-10)
    ...
    def calculate_completeness_summary(self) -> Dict:
        """
        Ultra-lightweight version that returns just a global summary
        Without system details (minimal memory usage)
        Ideal for dashboard or quick overview
        """
        query = """
        MATCH (ph:Phenomenon)
        WHERE ph.system IS NOT NULL

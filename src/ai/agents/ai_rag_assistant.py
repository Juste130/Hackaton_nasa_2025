...
from src.database.client import DatabaseClient, Publication, TextSection
...
class RAGAssistant(dspy.Module):
    ...
    def forward(self, question: str, conversation_context: Optional[List[Dict]] = None) -> Dict:
        ...
        print(" Citation tracker reset")
        #  Add conversation context to question if available
        context_question = question
        if conversation_context:
            last_interactions = conversation_context[-2:]  # Last 2 interactions
        ...
        try:
            ...
            try:
                ...
                # Ensure extracted_citations is always a list
                if not isinstance(extracted_citations, list):
                    extracted_citations = []
            except:
                extracted_citations = []
...
class RAGService:
    ...
    async def ask(
        ...
        # If PMCIDs are provided, enrich the question
        if pmcids:
        ...
        prediction_result = self.assistant(
        ...
        result = {
            'answer': prediction_result.answer if hasattr(prediction_result, 'answer') else str(prediction_result),
            'citations': prediction_result.citations if hasattr(prediction_result, 'citations') else [],
            'reasoning_trace': prediction_result.reasoning_trace if hasattr(prediction_result, 'reasoning_trace') else [],
            'tools_used': prediction_result.tools_used if hasattr(prediction_result, 'tools_used') else [],
            'confidence': prediction_result.confidence if hasattr(prediction_result, 'confidence') else 'medium'
        }
